...
def test_uniqueness_conflict_on_user_name(scim_client):
    scim_client.create(User(user_name="dupe"))
...
def test_search_filters_and_sorts(scim_client):
    scim_client.create(User(user_name="charlie", emails=[User.Emails(value="c@x.com")]))
    scim_client.create(
        User(user_name="alice", emails=[User.Emails(value="a@x.com", primary=True)])
    )
    response = scim_client.query(
        User, query_parameters=SearchRequest(filter="userName pr", sort_by="emails")
    )
...
def test_sort_puts_resources_without_a_value_last(scim_client):
    scim_client.create(
        User(user_name="has-email", emails=[User.Emails(value="a@x.com")])
    )
    scim_client.create(User(user_name="no-email"))
    response = scim_client.query(User, query_parameters=SearchRequest(sort_by="emails"))
...
def test_update_unknown_resource_raises():
    ...
    with pytest.raises(ResourceNotFoundError):
        storage.update(User, User(id="does-not-exist", user_name="ghost"))
